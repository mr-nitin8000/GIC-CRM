...
<?php
    ...
    //   $select = $conn->prepare('SELECT * FROM d_policy INNER JOIN owner_user ON d_policy.o_id = owner_user.o_id WHERE owner_user.o_id = ? and d_policy.ending_date >= now() ORDER BY `d_policy`.`ending_date` ASC');
      $select = $conn->prepare('SELECT * FROM d_policy INNER JOIN owner_user ON d_policy.o_id = owner_user.o_id WHERE owner_user.o_id = ? ORDER BY `d_policy`.`ending_date` DESC');
    ...
    <div class="table" id="element-3" style="margin: 5px; margin-bottom: 20px">
        <table class="tab1">
            ...
            <tr style="height: 40px">
                ...
                <td class="h2"><a
                        href="https://drive.google.com/file/d/<?php echo $row->down_policy;?>/view?usp=share_link"><button
